...
def save_signal(telegram_id, raw_signal, formatted_signal):
    conn = sqlite3.connect("signals.db")
    c = conn.cursor()
    signal_hash = get_signal_hash(raw_signal)
    try:
        c.execute('''
            INSERT INTO signals (telegram_id, signal_hash, received_at, raw_signal, formatted_signal)
            VALUES (?, ?, ?, ?, ?)
        ''', (telegram_id, signal_hash, datetime.now().isoformat(), raw_signal, formatted_signal))
        conn.commit()
...
def signal_hash_exists(signal_hash):
    conn = sqlite3.connect("signals.db")
    c = conn.cursor()
    c.execute('SELECT 1 FROM signals WHERE signal_hash = ?', (signal_hash,))
    exists = c.fetchone() is not None
    conn.close()
    return exists
...
def handle_signal(client, message):
    ...
    if text:
        signal_hash = get_signal_hash(text)
        if signal_hash_exists(signal_hash):
            print("Duplicate signal by hash, skipped (already sent).")
            return
